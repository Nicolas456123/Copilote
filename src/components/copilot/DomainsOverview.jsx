...
export default function DomainsOverview({ projects }) {
  ...
  return (
    <Card>
      <div className="text-sm font-bold text-navy mb-2.5">📊 VUE D'ENSEMBLE</div>
        ...
        {Object.entries(DOMAINS).map(([key, d]) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-sm font-bold text-navy mt-1">{d.label}</div>
              <div className="text-[13px] text-gray-400">{dp.length} projet{dp.length > 1 ? "s" : ""}</div>
              {dp.length > 0 && (
