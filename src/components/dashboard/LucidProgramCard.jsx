...
function RichText({ text }) {
  ...
  return (
    <div className="flex flex-col gap-2 text-[14px] text-navy leading-relaxed">
      {blocks.map((block, i) => {
...
function DreamSignsSection({ signs, onAdd, onRemove }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-gray-100">
      <div className="text-[13px] font-bold text-navy mb-1.5">🧭 MES DREAM SIGNS ({signs.length})</div>
      {signs.length > 0 && (
        <div className="flex flex-col gap-1 mb-2">
          {signs.map(s => {
            const cat = DREAM_SIGN_CATEGORIES.find(c => c.id === s.category);
            return (
              <div key={s.id} className="flex items-center gap-1.5 text-[13px] py-1 px-2 rounded-lg bg-[#fafafa]">
                ...
                <button
                  ...
                  className="text-gray-300 hover:text-coral bg-transparent border-none cursor-pointer text-[14px] leading-none"
                  title="Supprimer"
                >×</button>
              </div>
            );
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex gap-1.5">
          <select
            ...
            className="text-[13px] py-1 px-2 rounded-lg border border-gray-200 bg-white font-nunito"
          >
...
function KnowledgeBox({ title, children }) {
  ...
  return (
    <div className="mt-2 border border-gray-100 rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full text-left py-2 px-3 bg-[#f5f2ee] text-[13px] font-bold text-navy cursor-pointer font-nunito border-none flex justify-between"
      >
        <span>{title}</span>
...
function TaskRow({ task, done, onToggle, count }) {
  ...
  return (
    ...
    >
      ...
      {count !== undefined && (
        <span className={`text-[12px] font-semibold tabular-nums ${done ? 'text-sage' : 'text-gray-400'}`}>
          {count}{task.target ? `/${task.target}` : ''}
        </span>
      )}
      {isAuto && count === undefined && (
        <span className="text-[11px] text-gray-400 italic">auto</span>
      )}
    </button>
  );
}
  ...
  if (!state.started) {
    return (
      <Card>
        ...
        <div className="text-[14px] text-navy leading-relaxed mb-3">
        ...
        <div className="grid grid-cols-2 gap-1.5 mb-3">
          {LUCID_PROGRAM.map(w => (
            <div key={w.week} className="text-[12px] text-navy py-1.5 px-2 rounded-lg bg-[#f5f2ee]">
    ...
    <Card>
      ...
      >
        <div className="text-left">
          ...
          <div className="text-[13px] text-navy mt-0.5">{weekData.title}</div>
        </div>
        <div className="flex items-center gap-2">
          <span className={`text-[12px] font-semibold ${allDoneToday ? 'text-sage' : 'text-gray-400'}`}>
      ...
      {open && (
        <div className="mt-3 animate-fade-in">
          ...
          <div className="text-[13px] text-gray-500 italic mb-2 leading-relaxed">{weekData.focus}</div>
          ...
          {weekData.tip && (
            <div className="text-[13px] text-navy py-2 px-3 rounded-xl bg-sand/30 mb-2 leading-relaxed">
            ...
            <KnowledgeBox title="🎯 Reality checks classés par fiabilité">
              <div className="flex flex-col gap-1.5">
                {REALITY_CHECK_RANKING.map((r, i) => (
                  <div key={r.name} className="text-[13px] py-1.5 px-2 rounded-lg bg-[#fafafa]">
                    <div className="flex justify-between text-navy font-semibold">
                      <span>{i + 1}. {r.name}</span>
                      <span className="text-coral text-[12px]">{r.score}</span>
                    </div>
                    <div className="text-gray-500 text-[12px] leading-relaxed mt-0.5">{r.note}</div>
                  </div>
                ))}
              </div>
            </KnowledgeBox>
          ...
          <div className="text-[12px] text-gray-400 leading-relaxed mt-3 pt-2 border-t border-gray-100">
